...
- **Proper cleanup**: DNS records are removed when a subdomain or its server is deleted.
- **Allocation changes at any time**; the allocation is set to `null` (Unknown) after a server
  transfer so the user can re-point it.
